...
CSV_FILE      = "smart_home_log.csv"
TEXT_FILE     = "smart_home_report.txt"
ALERT_FILE    = "smart_home_alerts.txt"
GRAPH_FILE    = "smart_home_dashboard.png"

THRESHOLDS = {
    "temperature_high" : 30.0,   # °C  → Turn ON AC
    "temperature_low"  : 26.0,   # °C  → Turn ON Heater
    "humidity_high"    : 58.0,   # %   → Turn ON Dehumidifier
    "humidity_low"     : 50.0,   # %   → Turn ON Humidifier
    "light_low"        : 400.0,  # lux → Turn ON Indoor Lights
    "light_high"       : 430.0,  # lux → Close Window Blinds
}
...
# Device state memory — tracks ON/OFF for every device
# Alerts only fire when a device actually CHANGES state
_device = {
    "ac"           : False,
    "heater"       : False,
    "dehumidifier" : False,
...
def simulate_humidity():
    ...
    # Device feedback — humidifier raises humidity, dehumidifier lowers it
    if _device["humidifier"]:
        _state["humidity"] += 1.5
    if _device["dehumidifier"]:
        _state["humidity"] -= 1.5
...
def simulate_light():
    if random.random() < 0.03:
        _state["light"] += random.choice([-60.0, 60.0])   # reduced spike magnitude
    else:
...
def _alert(sensor, condition, value, threshold, action, severity, icon):
    """Helper to build a single alert dict."""
    return {
        "sensor"    : sensor,
        "condition" : condition,
        "value"     : value,
...
def evaluate_rules(reading):
    ...
    Each device has state memory — an alert fires ONLY when a
    device changes state (OFF→ON or ON→OFF), not every reading.

    Severity levels:
        CRITICAL  → immediate danger / urgent action
        WARNING   → abnormal but not dangerous
        INFO      → informational device action
    """
    global _device
    alerts = []
    ...
    if t > THRESHOLDS["temperature_high"]:
        if not _device["ac"]:                          # just turned ON
            _device["ac"] = True
            _device["heater"] = False
            alerts.append(_alert(
                "Temperature", f"> {THRESHOLDS['temperature_high']} °C",
                f"{t} °C", THRESHOLDS["temperature_high"],
                "Turn ON Air Conditioner", "CRITICAL", "🔴"
            ))
    elif t < THRESHOLDS["temperature_low"]:
        if not _device["heater"]:                      # just turned ON
            _device["heater"] = True
            _device["ac"] = False
            alerts.append(_alert(
                "Temperature", f"< {THRESHOLDS['temperature_low']} °C",
                f"{t} °C", THRESHOLDS["temperature_low"],
                "Turn ON Heater", "WARNING", "🔵"
            ))
    else:                                              # comfortable range
        if _device["ac"]:                              # AC was ON → turn OFF
            _device["ac"] = False
            alerts.append(_alert(
                "Temperature", "back to normal",
                f"{t} °C", None,
                "Turn OFF Air Conditioner", "INFO", "❄️"
            ))
        if _device["heater"]:                          # Heater was ON → turn OFF
            _device["heater"] = False
            alerts.append(_alert(
                "Temperature", "back to normal",
                f"{t} °C", None,
                "Turn OFF Heater", "INFO", "🌡️"
            ))

    # ── Humidity ───────────────────────────────────────────
    if h > THRESHOLDS["humidity_high"]:
        if not _device["dehumidifier"]:                # just turned ON
            _device["dehumidifier"] = True
            _device["humidifier"] = False
            alerts.append(_alert(
                "Humidity", f"> {THRESHOLDS['humidity_high']} %",
                f"{h} %", THRESHOLDS["humidity_high"],
                "Turn ON Dehumidifier", "WARNING", "💧"
            ))
    elif h < THRESHOLDS["humidity_low"]:
        if not _device["humidifier"]:                  # just turned ON
            _device["humidifier"] = True
            _device["dehumidifier"] = False
            alerts.append(_alert(
                "Humidity", f"< {THRESHOLDS['humidity_low']} %",
                f"{h} %", THRESHOLDS["humidity_low"],
                "Turn ON Humidifier", "WARNING", "🏜️"
            ))
    else:
        if _device["dehumidifier"]:                    # turned OFF
            _device["dehumidifier"] = False
            alerts.append(_alert(
                "Humidity", "back to normal",
                f"{h} %", None,
                "Turn OFF Dehumidifier", "INFO", "✅"
            ))
        if _device["humidifier"]:
            _device["humidifier"] = False
            alerts.append(_alert(
                "Humidity", "back to normal",
                f"{h} %", None,
                "Turn OFF Humidifier", "INFO", "✅"
            ))

    # ── Light ──────────────────────────────────────────────
    if l < THRESHOLDS["light_low"]:
        if not _device["lights"]:                      # just turned ON
            _device["lights"] = True
            _device["blinds"] = False
            alerts.append(_alert(
                "Light", f"< {THRESHOLDS['light_low']} lux",
                f"{l} lux", THRESHOLDS["light_low"],
                "Turn ON Indoor Lights", "INFO", "💡"
            ))
    elif l > THRESHOLDS["light_high"]:
        if not _device["blinds"]:                      # just closed
            _device["blinds"] = True
            _device["lights"] = False
            alerts.append(_alert(
                "Light", f"> {THRESHOLDS['light_high']} lux",
                f"{l} lux", THRESHOLDS["light_high"],
                "Close Window Blinds", "INFO", "☀️"
            ))
    else:
        if _device["lights"]:                          # lights no longer needed
            _device["lights"] = False
            alerts.append(_alert(
                "Light", "back to normal",
                f"{l} lux", None,
                "Turn OFF Indoor Lights", "INFO", "✅"
            ))
        if _device["blinds"]:
            _device["blinds"] = False
            alerts.append(_alert(
                "Light", "back to normal",
                f"{l} lux", None,
                "Open Window Blinds", "INFO", "✅"
            ))

    # ── Motion ─────────────────────────────────────────────
    # Motion is event-based (not state-based) — alert every detection
    if m == 1:
        alerts.append(_alert(
            "Motion", "== DETECTED",
            "DETECTED", None,
            "Activate Security Light & Notify", "CRITICAL", "🚨"
        ))
...
def init_text():
    ...
    with open(TEXT_FILE, "w", encoding="utf-8") as f:
        f.write("\n")
        f.write(f"{'╔' + '═' * BOX_WIDTH + '╗'}\n")
        f.write(f"║{'SMART HOME MONITORING SYSTEM':^{BOX_WIDTH}}║\n")
        f.write(f"║{'Decodelabs IoT Internship - Task 5':^{BOX_WIDTH}}║\n")
...
def print_banner():
    print("\n" + "═" * 62)
    print(color(f"{'SMART HOME MONITORING SYSTEM — Task 5':^62}", "BOLD"))
    print("═" * 62)
...
def plot_dashboard(readings, all_alerts_list):
    ...
    fig.suptitle(
        "Smart Home Monitoring System — Task 5",
...
# =========================================================
# SYSTEM HEALTH REPORT  (Task 5 addition)
# Analyses the full session and prints a plain-English verdict
# =========================================================

def system_health_report(readings, all_alerts_list):
    """
    Summarise the monitoring session in plain English.
    Verdict is determined by the highest severity seen:
        CRITICAL  → at least one CRITICAL alert fired
        WARNING   → at least one WARNING alert (no CRITICAL)
        NORMAL    → no alerts at all
    Also identifies which sensor triggered the most alerts.
    Prints to terminal and appends to the text report.
    """
    ...
    total_alerts   = sum(len(b) for b in all_alerts_list)
    critical_count = sum(1 for b in all_alerts_list for a in b
                         if a["severity"] == "CRITICAL")
    warning_count  = sum(1 for b in all_alerts_list for a in b
                         if a["severity"] == "WARNING")

    # Determine overall verdict
    if critical_count > 0:
        verdict     = "CRITICAL"
        verdict_clr = "CRITICAL"
        advice      = "Immediate attention required — check sensor logs."
    elif warning_count > 0:
        verdict     = "WARNING"
        verdict_clr = "WARNING"
        advice      = "Environment out of comfort range — review thresholds."
    else:
        verdict     = "NORMAL"
        verdict_clr = "GREEN"
        advice      = "All sensors stayed within safe limits."

    most_active = (
        max(sensor_counts, key=lambda k: sensor_counts.get(k, 0))
    if sensor_counts else "None"
    )

    # ── Terminal output ──────────────────────────────────
    print("\n" + "═" * 62)
    print(color(f"{'SYSTEM HEALTH REPORT':^62}", "BOLD"))
    print("═" * 62)
    print(f"  Overall Status  : {color(verdict, verdict_clr)}")
    print(f"  Advice          : {advice}")
    print(f"  Total Alerts    : {total_alerts}  "
          f"(Critical: {critical_count}, Warning: {warning_count})")
    print(f"  Most Active     : {most_active} sensor"
          f"  ({sensor_counts.get(most_active, 0)} alerts)")
    if sensor_counts:
        print("\n  Alert Breakdown by Sensor:")
        for sensor, count in sorted(sensor_counts.items(), key=lambda x: -x[1]):
            bar = "█" * count
            print(f"    {sensor:<14}: {bar}  ({count})")
    print("═" * 62 + "\n")

    # ── Append to text report ────────────────────────────
    with open(TEXT_FILE, "a", encoding="utf-8") as f:
        f.write(f"\n{'╔' + '═' * BOX_WIDTH + '╗'}\n")
        f.write(f"║{'SYSTEM HEALTH REPORT':^{BOX_WIDTH}}║\n")
        f.write(f"{'╠' + '═' * BOX_WIDTH + '╣'}\n")
        f.write(box_line("Overall Status",  verdict))
        f.write(box_line("Advice",          advice))
        f.write(box_line("Total Alerts",    total_alerts))
        f.write(box_line("Critical Alerts", critical_count))
        f.write(box_line("Warning Alerts",  warning_count))
        f.write(box_line("Most Active",     f"{most_active} ({sensor_counts.get(most_active,0)} alerts)"))
        f.write(f"{'╚' + '═' * BOX_WIDTH + '╝'}\n")
...
if __name__ == "__main__":
    ...
    system_health_report(all_readings, all_alerts_list)
